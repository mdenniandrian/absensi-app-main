        ...
        @if ($attendance->data->is_using_qrcode && !$data['is_there_permission'])
            ...
            @if ($attendance->data->is_start && !$data['is_has_enter_today'])
                ...
                <a href="{{ route('home.permission', $attendance->id) }}"
                    class="btn btn-info px-3 py-2 btn-sm fw-bold d-block w-100 mb-1">Izin Kerja</a>
                <a href="{{ route('home.permissionSakit', $attendance->id) }}"
                    class="btn btn-danger px-3 py-2 btn-sm fw-bold d-block w-100 mb-1">Izin Sakit</a>
                <a href="{{ route('home.permissionCuti', $attendance->id) }}"
                    class="btn btn-warning px-3 py-2 btn-sm fw-bold d-block w-100">Izin Cuti</a>
            @endif
